...
let studentName='', currentQ=0, QUESTIONS=[], WORD_TASKS=[], EXCEL_TASKS=[], PPT_TASKS=[];
let userAnswers=[], mcqScore=0, timerInterval=null, timeLeft=2700;
let wordFileUploaded=false, excelFileUploaded=false, pptFileUploaded=false;
let wordFileName='', excelFileName='', pptFileName='';
let wordFileData=null, excelFileData=null, pptFileData=null;
...
(function initParticles(){
  ...
  function createDots(n){
    ...
    for(let i=0;i<n;i++)dots.push({
      ...
      a:Math.random()*.5+.08,
      hue: Math.random()<.5 ? 220 : 280  // blue or purple
    });
  }
  function draw(){
    ...
    dots.forEach(d=>{
      ...
      ctx.fillStyle=`hsla(${d.hue},90%,70%,${d.a})`;ctx.fill();
    });
    for(let i=0;i<dots.length;i++)for(let j=i+1;j<dots.length;j++){
      ...
      if(dist<110){
        ...
        ctx.strokeStyle=`rgba(79,142,247,${.1*(1-dist/110)})`;ctx.lineWidth=.5;ctx.stroke();
      }
    }
    requestAnimationFrame(draw);
  }
  resize();createDots(65);draw();
...
async function init(){
  try {
    ...
    listenExamConfig(cfg=>{if(cfg)examConfig={...DEFAULT_CONFIG,...cfg};populateLandingConfig();});
...
async function startExam(){
  ...
  }catch(e){console.warn('Could not check submissions:',e.message);}
  btn.disabled=false;btn.querySelector('span').textContent='Start Exam';
  ...
  if(c.selectionMode==='manual'){
    ...
    const pt    =buildManualTasks(c.manualPptTasks||[],window.PPT_TASK_BANK||[]);
  ...
  buildTaskList('word-tasks',WORD_TASKS,false,'word');
  buildTaskList('excel-tasks',EXCEL_TASKS,false,'excel');
  buildTaskList('ppt-tasks',PPT_TASKS,false,'ppt');
  setupDragDrop('word-upload-zone','word-file','word');
  setupDragDrop('excel-upload-zone','excel-file','excel');
  setupDragDrop('ppt-upload-zone','ppt-file','ppt');
...
function sectionColor(s){
  if(s==='Hardware')return{bg:'rgba(167,139,250,.1)',text:'#a78bfa',border:'rgba(167,139,250,.22)'};
  if(s==='MS Word') return{bg:'rgba(79,142,247,.1)', text:'#6ba3ff',border:'rgba(79,142,247,.22)'};
  if(s==='MS Excel')return{bg:'rgba(251,146,60,.1)', text:'#fb923c',border:'rgba(251,146,60,.22)'};
  if(s==='Software')return{bg:'rgba(52,211,153,.1)', text:'#34d399',border:'rgba(52,211,153,.22)'};
  return{bg:'rgba(251,146,60,.1)',text:'#fb923c',border:'rgba(251,146,60,.22)'};
}
...
function buildTaskList(containerId, tasks, isExcel, type){
  const container=document.getElementById(containerId);container.innerHTML='';
  tasks.forEach((t,idx)=>{
    const div=document.createElement('div');
    const extraClass = type==='excel' ? ' excel-ctx-task' : type==='ppt' ? ' ppt-ctx-task' : '';
    div.className='task-item'+extraClass;
...
function handleFileUpload(type, input){
  ...
  reader.onload=function(e){
    ...
    else if(type==='excel'){excelFileUploaded=true;excelFileName=file.name;excelFileData=b64;}
    else if(type==='ppt'){pptFileUploaded=true;pptFileName=file.name;pptFileData=b64;}
    ...
    zone.style.borderColor='var(--green)';zone.style.background='rgba(52,211,153,.04)';
...
async function submitExam(){
  ...
  if(!pptFileUploaded)missing.push('PowerPoint (.pptx)');
  if(missing.length>0){
    if(!confirm(`⚠ You haven't uploaded your ${missing.join(', ')} file${missing.length>1?'s':''}.\n\nSubmit anyway?`))return;
  }
  examSubmitted=true;
  mcqScore=0;
  ...
  const submission={
    ...
    wordTasks:WORD_TASKS, excelTasks:EXCEL_TASKS, pptTasks:PPT_TASKS,
    examConfig:examConfig||DEFAULT_CONFIG,
    graded:false, wordGrade:null, excelGrade:null, pptGrade:null,
    totalGrade:null, teacherNote:''
  };
  ...
  try{
    ...
    document.getElementById('sc-ppt').textContent=pptFileUploaded?`✅ ${pptFileName}`:'⚠ Not uploaded';
...
function buildManualQuestions(ids){
  return ids.map(id=>{const q=window.QUESTION_BANK.find(q=>q.id===id);return q?window.prepareQuestion(q):null;}).filter(Boolean);
}
